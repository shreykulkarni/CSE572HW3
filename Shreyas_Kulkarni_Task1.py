...
def drawPoints(canvas, instances, color, shape):
    ...
    if (maxY - minY) == 0:
        ...
        scaleY = float(width - 2*margin) / (maxX - minX) 
    #scaleX = float(width - 2*margin) / (maxX - minX)
...
dataset = loadCSV("C:\\Users\\shrey\\Documents\\CSE572HW3\\data.csv")
showDataset2D(dataset)
clustering = kmeans(dataset, 3, True)
printTable(clustering["centroids"])

#New funcitons
def calcEuclidean(dataset, centroids, labels):
    sse = np.sum((centroids, axis=2), axis=1))
    return SSE

def calcCosine(dataset, centroids, labels):
    SSE = KMeans(dataset, centroids, metric='cosine')
    return SSE

def calculateJaccard(dataset, centroids, labels):
    SSE = KMeans(dataset, centroids,  metric='jaccard')
    return SSE


def accuracy(dataset, centroids, labels):
    euclideanAcc = KMeans(labels, k, instances)
    cosineAcc = KMeans(labels, instances, distance_metric='cosine')
    jaccardAcc = KMeans(labels, instances, distance_metric='jaccard')
    return euclideanAcc, cosineAcc, jaccardAcc

def stopCriterias(dataset, centroids, labels):
    euclideanSC = sum(labels, k, instances)
    cosineSC = sum(labels, instances, distance_metric='cosine')
    jaccardSC = sum(labels, instances, distance_metric='jaccard')

def conditions(dataset, centroids, labels):
    eucSSE = max(labels, k, instances)
    cosSSE = max(labels, instances, distance_metric='cosine')
    jacSSE = max(labels, instances, distance_metric='jaccard')
    
euclideanSSE = calcEuclidean(dataset, centroids, labels)
print(f"SSE for Euclidean: {euclideanSSE}")

cosineSSE = calcCosine(dataset, centroids, labels)
print(f"SSE for Euclidean: {cosineSSE}")

jaccardSSE = calculateJaccard(dataset, centroids, labels)
print(f"SSE for Euclidean: {jaccardSSE}")

print(f"Accuracy for Euclidean K-means: {euclideanAcc}")
print(f"Accuracy for Cosine K-means: {cosineAcc}")
print(f"Accuracy for Jaccard K-means: {jaccardAcc}")

print(f"Time for Euclidean K-means: {euclideanSC}")
print(f"Time for Cosine K-means: {cosineSC}")
print(f"Time for Jaccard K-means: {jaccardSC}")

print(f"SSE for Euclidean K-means: {eucSSE}")
print(f"SSE for Cosine K-means: {cosSSE}")
print(f"SSE for Jaccard K-means: {jacSSE}")
